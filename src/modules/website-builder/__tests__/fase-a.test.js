...
describe('existing GS routes still load (smoke)', () => {
  let app;

  before(() => {
    const authRoutes = require('../../auth/auth.routes');
    const licensesRoutes = require('../../licenses/licenses.routes');
    app = express();
    app.use(express.json());
    app.use(
      session({
  ...
  it('GET /api/auth/me still responds (401 when anonymous)', async () => {
    const res = await request(app).get('/api/auth/me').set('Accept', 'application/json');
    assert.equal(res.status, 401);
    assert.ok(res.body.error);
  });

  it('POST /api/licenses/validate still responds without a code', async () => {
    const res = await request(app).post('/api/licenses/validate').send({});
    assert.equal(res.status, 400);
    assert.equal(res.body.valid, false);
  });

  it('api aggregator still exports auth and licenses', () => {
    const api = require('../../../routes/api');
    assert.equal(typeof api, 'function');
  });
});

describe('feature flag off does not mount builder routes', () => {
  it('keeps /api/website-builder as 404 when the module is not mounted', async () => {
    const app = express();
    app.use(express.json());
    ...
    const res = await request(app).get('/api/website-builder/health');
    assert.equal(res.status, 404);

    const me = await request(app).get('/api/auth/me').set('Accept', 'application/json');
